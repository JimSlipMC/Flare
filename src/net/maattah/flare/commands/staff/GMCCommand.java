		...
		if(cmd.getName().equalsIgnoreCase("gmc")) {
			...
			if(p.hasPermission(Main.getNode() + ".gamemode")) {
				if(args.length == 0) {
					...
					p.sendMessage(Lang.PREFIX.toString() + Lang.GAMEMODE_CREATIVE.toString());
					return true;
				}
					...
					if(p.hasPermission(Main.getNode() + ".gamemode.other")) {
						...
						if(target != null) {
							...
							p.sendMessage(Lang.PREFIX.toString() + Lang.GAMEMODE_CREATIVE_OTHER.toString().replace("%player%", target.getName()));
							target.sendMessage(Lang.PREFIX.toString() + Lang.GAMEMODE_CREATIVE_OTHER2.toString());
							return true;
						}
					}
				} else {
